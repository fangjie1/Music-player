...
export default {
  ...
  methods: {
    ...
    showLyric () {
      // 监听播放audio进度, 切换歌词显示
      this.$refs.audio.$refs.audio.addEventListener("timeupdate", this.Fn)
    },
    Fn () {
      let curTime = Math.floor(this.$refs.audio.$refs.audio.currentTime);
      // 避免空白出现
  ...
  mounted () {
    if (this.isRecommendMusic) {
      // 随机选一首播放
      recommendGetPlaylistAPI({ id: this.id, limit: 10, offset: Math.random(0, 10) * 5 }).then(data => {
        const songs = data.data.songs
        const index = Math.floor(Math.random() * 9) + 1;
        this.getSong(songs[index].id);
      })
    } else {
  ...
  beforeDestroy () {
    this.$refs.audio.$refs.audio.removeEventListener("timeupdate", this.Fn);
  }
};
</script>
